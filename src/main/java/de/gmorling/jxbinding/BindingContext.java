 ...
 *
 */
public class BindingContext implements ListChangeListener<ConstraintViolation<?>> {

	private final Set<Binding<?, ?>> bindings = new HashSet<Binding<?, ?>>();

	private final ObjectProperty<ObservableList<ConstraintViolation<?>>> allConstraintViolations = new SimpleObjectProperty<ObservableList<ConstraintViolation<?>>>(FXCollections.<ConstraintViolation<?>>observableArrayList());
	
	private final BooleanProperty isValidProperty = new SimpleBooleanProperty(true);
    ...
    public ObjectProperty<ObservableList<ConstraintViolation<?>>> constraintViolationsProperty() {
    ...
    public static class ReadOnlyBindingBuilderContext<M> {
		...
		public <T> ReadOnlyBindingBuilderConverterContext<M, T> withConverter(Converter<M, T> converter) {
			return new ReadOnlyBindingBuilderConverterContext<M, T>(modelProperty, converter, bindingContext, targetUpdatePolicy);
		}
		...
		public <T> Binding<M, T> to(Property<T> targetProperty) {
			...
			Binding<M, T> binding = new Binding<M, T>(modelProperty, targetProperty, targetUpdatePolicy, converter, validator);
    ...
    public static class BindingBuilderContext<M> {
		...
		public <T> BindingBuilderConverterContext<M, T> withConverter(Converter<M, T> converter) {
			return new BindingBuilderConverterContext<M, T>(modelProperty, converter, bindingContext, modelUpdatePolicy, targetUpdatePolicy);
		}
		...
		public <T> Binding<M, T> to(Property<T> targetProperty) {
			...
			Binding<M, T> binding = new Binding<M, T>(modelProperty, targetProperty, modelUpdatePolicy, targetUpdatePolicy, converter, validator);
			bindingContext.addBinding(binding);
		...
		public <T> Binding<M, T> to(ReadOnlyProperty<T> targetProperty) {
			...
			Binding<M, T> binding = new Binding<M, T>(modelProperty, targetProperty, modelUpdatePolicy, converter, validator);
			bindingContext.addBinding(binding);
    ...
    public static class ReadOnlyBindingBuilderConverterContext<M, T> {
		...
		public ReadOnlyBindingBuilderConverterContext(ReadOnlyProperty<M> modelProperty, Converter<M, T> converter, BindingContext bindingContext, UpdatePolicy targetUpdatePolicy) {
			this.modelProperty = modelProperty;
			this.bindingContext = bindingContext;
    ...
    public static class BindingBuilderConverterContext<M, T> {
		...
		public BindingBuilderConverterContext(Property<M> modelProperty, Converter<M, T> converter, BindingContext bindingContext, UpdatePolicy modelUpdatePolicy, UpdatePolicy targetUpdatePolicy) {
			this.modelProperty = modelProperty;
			this.bindingContext = bindingContext;
			this.converter = converter;
			this.modelUpdatePolicy = modelUpdatePolicy;
	...
	@Override
	public void onChanged(ListChangeListener.Change<? extends ConstraintViolation<?>> c) {
//		c.wasAdded()
//		c.getAddedSize()
		
		
		Set<ConstraintViolation<?>> allViolations = new HashSet<ConstraintViolation<?>>();
		
		for (Binding<?, ?> oneBinding : bindings) {
			allViolations.addAll(oneBinding.targetConstraintViolationsProperty().getValue());
		}
		
		allConstraintViolations.getValue().clear();
		allConstraintViolations.getValue().addAll(allViolations);
		
		System.out.println(allConstraintViolations);
